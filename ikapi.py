...
class IKApi:
    def __init__(self, args, storage):
        ...
        self.maxresults = args.maxresults  # New parameter to limit total results
    ...
    def save_search_results(self, q):
        ...
        current = 1
        docids = []
        max_results = self.maxresults if hasattr(self, 'maxresults') and self.maxresults else float('inf')
        
        while current <= max_results:  # Stop if we've reached max_results
            results = self.search(q, pagenum, self.maxpages)
            obj = json.loads(results)
            ...
            docs = obj['docs']
            self.logger.warning('Num results: %d, pagenum: %d found: %s q: %s', 
                              len(docs), pagenum, obj['found'], q)
            
            for doc in docs:
                if current > max_results:  # Stop if we've reached max_results
                    break
            ...
            # Stop if we've reached the end of results or max_results
            if len(docs) < self.maxpages or current > max_results:
                break
...
def get_arg_parser():
    ...
    parser.add_argument('-r', '--maxresults', type = int, dest='maxresults', \
                        action='store', default = 0, required = False, \
                        help='maximum number of results to fetch (0 for no limit)')
    return parser
