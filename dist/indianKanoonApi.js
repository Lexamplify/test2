...
const API_TOKEN = process.env.INDIANKANOON_TOKEN;
export async function searchIndianKanoon(title) {
    const queries = [
        title,
    ...
    for (const query of queries) {
        try {
            ...
            if (results && results.length > 0) {
                const bestMatch = results.find((doc) => doc.title.toLowerCase().includes('nanavati')) || results[0];
